...
void *Cozinheiro::Semaforo(void *v) {
    ...
    while (fecho) {
        ...
        if (Restaurante::AvisaFechamento() && *(int *) &sem_pedido == 0) {
            fecho = false;
        }
...
string Cozinheiro::bufferInicio(const Pedido &pedido) {
    string buffer;
    buffer.append(currentDateTime()).append(":\t").append("Cozinheiro ").append(
            to_string(pedido.getCozinhero())).append(
...
string Cozinheiro::bufferFinal(const Pedido &pedido) {
    string buffer;
    buffer.append(currentDateTime()).append(":\t").append("Cozinheiro ").append(
            to_string(pedido.getCozinhero())).append(" Entregando pedido da Mesa ").append(
